        ...
        <fieldset>
            <table class="form-table">
                <tbody>
                    ...
                    <tr>
                        <th scope="row">
                            <label for='wpbiztextc7-admin-attach'>Send Attachment URL <div class="wpbiztextc7-tooltip"><span class="wpbiztextc7-tooltiptext">An attachment is sent as a url and stored in media. If this is enabled, will be added to the end of the message.</span></div><label>
                        </th>
                        <td>
                            <input type="checkbox" id='wpbiztextc7-admin-attach' name='<?= $options_name . '[wpbiztextc7-admin-attach]'?>' value='1' <?php checked( 1, $options['wpbiztextc7-admin-attach'] ); ?> /> <span>Use if file attachment is used in form</span>
                        </td>
                    </tr>
                </tbody>
            </table>
...
function wpbiztextc7_save_contact_form($form) {
    ...
    if (isset( $_POST[$options_name])){
        ...
        $options['wpbiztextc7-admin-attach'] =  trim(sanitize_text_field($_POST[$options_name]['wpbiztextc7-admin-attach']));
...
function wpbiztextc7_before_send_email($form, &$abort, $submission){
   ...
   function wpbiztextc7_create_attachment($filename){
    ...
    // should skip saving to media library?
    ...
    do_action('wpbiztextc7_create_attachment_id_generated', $attach_id);
    return $attach_data;
    }
            ...
            foreach ($uploaded_files as $fieldName => $filepath) {
                ...
                if (is_array($filepath)) {
                    foreach ($filepath as $key => $value) {
                        $data = wpbiztextc7_create_attachment($value);
                    }
                } else {
                    $data = wpbiztextc7_create_attachment($filepath);
                    
                }
                if (isset($data)){
					$wp_upload_dir = wp_upload_dir();
					array_push($attachemnt_url,$wp_upload_dir['baseurl'].'/'.$data['file']);
				} 
            }
        }
    }
    ...
    if ($raw_customer_phone != '' && $customer_auto_response != '' ){
        ...
        if ($options['wpbiztextc7-send-autoreply-only'] == 1) {
            ...
            //add attachments by url
            $attachemnt_url_string = "";
            $add_atach_url = "";
            
            if ($options['wpbiztextc7-admin-attach'] == 1 && !empty($attachemnt_url)){
            
              foreach ($attachemnt_url as $url) {
                $attachemnt_url_string .= $url;
              }
              
              $add_atach_url = "Attached:". $attachemnt_url_string;
            ...
            $bizTextData = array(
                'websiteId' => $bizTextId,
                'txt' => $admin_message . " " . $add_atach_url,
...
add_filter('wpcf7_feedback_response', 'wpbiztextc7_ajax_json_echo', 10, 2);
...
function wpbiztextc7_ajax_json_echo($response, $result){
    ...
    if ($options['wpbiztextc7-email-notification-activation'] == 1){
        if ($raw_email_notif_number != '' && ($response['status'] == 'mail_sent' || $response['status'] == 'mail_failed')){
            ...
            $bizTextData = array(
                'websiteId' => $bizTextId,
                'txt' => $msg_came_from . $txt . " " . $add_atach_url,
